...
<body>
    <main>
        ...
        <form action="calculator.php" method="get">
            <input type="text" name="suchfeld" id="seach">
            <input type="submit" value="suchen" id="send" name="suche_enter">
            <select name="rabatt" id="rabatt">
                <option value="_0p">0% (Standart)</option>
                <option value="_5p">5% (Mitarbeiter)</option>
                <option value="_10p">10% (Besonders)</option>
                <option value="_20p">Rohbau (Nur für eigenbedarf))</option>
            </select>
        </form>
        <section>
            ...
            if (isset($_GET["suche_enter"])) {
                ...
                switch ($_GET["rabatt"]) {
                    case '_0p':
                        $GLOBALS["draufrechner"] = (1.18) * $steuern;
                        break;
                    case '_5p':
                        $GLOBALS["draufrechner"] = (1.13) * $steuern;
                        break;
                    case '_10p':
                        $GLOBALS["draufrechner"] = (1.08) * $steuern;
                        break;
                    case '_20p':
                        $GLOBALS["draufrechner"] = (1);
                        break;
                    default:
                }
                $suchbegriff = trim(htmlentities($_GET["suchfeld"]));
                if ($suchbegriff !== "") {
                    $stmtFullcar = $mysql->prepare("SELECT Fullcar FROM carlist WHERE Fullcar LIKE '%$suchbegriff%' ORDER BY Fullcar limit 10");
                    $stmtFullcar->execute();
                    while ($rowFullcar = $stmtFullcar->fetch()) {
                        $car = $rowFullcar["Fullcar"];
            ...
            ?>
                            <details>
                                <summary class='carBut'><?php echo "<div>" . $car . "</div>"; ?> </summary>
                                ...
                                <article class="prices">
                                    ...
                                    if ($rubyfull == 0 && $diamondfull != 0) {
                                    ?>
                                        <h2>Endsumme (ohne Motor): <?php echo number_format((($fullprice * $draufrechner) + $rubyfull + $diamondfull), 2, ".", "") ?></h2>
                                        <h2>Diamanten: <?php echo $diamondfull ?></h2>
                                        <h2>Zusammenbau: <?php echo number_format($fullprice * $draufrechner, 2, ".", "") ?></h2>
                                    <?php
                                    } else if ($diamondfull == 0 && $rubyfull != 0) {
                                    ?>
                                        <h2>Endsumme (ohne Motor): <?php echo number_format((($fullprice * $draufrechner) + $rubyfull + $diamondfull), 2, ".", "") ?></h2>
                                        <h2>Rubine: <?php echo $rubyfull ?></h2>
                                        <h2>Zusammenbau: <?php echo number_format($fullprice * $draufrechner, 2, ".", "") ?></h2>
                                    <?php
                                    } else if ($diamondfull != 0 && $rubyfull != 0) {
                                    ?>
